...
export default function WhatsAppSimulator({
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-4 right-4 z-50 w-[calc(100vw-2rem)] max-w-[350px] bg-slate-900 rounded-3xl shadow-2xl border border-slate-800 overflow-hidden flex flex-col h-[440px] max-h-[70vh] text-white"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Smartphone Top Header Bar */}
        <div className="bg-emerald-900/90 backdrop-blur-md p-3 border-b border-emerald-800/60 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="relative w-9 h-9 rounded-full bg-emerald-600 flex items-center justify-center font-bold text-white border border-emerald-400/40 shrink-0">
              {activeChannel === 'client' ? <User className="w-4 h-4" /> : <Store className="w-4 h-4" />}
            </div>
            <div>
        ...
        <div className="bg-slate-950 p-1.5 border-b border-slate-800 flex space-x-2">
          <button
            onClick={() => setActiveChannel('client')}
            className={`flex-1 py-1 rounded-xl text-xs font-bold transition-all text-center cursor-pointer ${
        ...
        {/* Chat Messages Area */}
        <div className="flex-1 p-3 overflow-y-auto space-y-2.5 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:16px_16px] bg-slate-950">
          {filteredMessages.length === 0 ? (
            <div className="h-full flex items-center justify-center text-center text-slate-500 text-xs p-4">
              <div>
                <MessageSquare className="w-7 h-7 mx-auto mb-2 text-slate-600" />
            ...
            filteredMessages.map((msg) => {
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[88%] rounded-2xl p-2.5 text-xs leading-relaxed shadow-xs ${
                      isAgent
                  ...
                  >
                    ...
                    {msg.order_id && msg.text.includes('Lien de paiement') && (
                      <button
                        ...
                        className="mt-2 w-full py-1 px-2 bg-cyan-500 hover:bg-cyan-600 text-slate-950 font-extrabold text-[10px] rounded-lg shadow-2xs flex items-center justify-center space-x-1 cursor-pointer"
                      >
                        <span>👉 Régler maintenant (Wave / OM)</span>
                      </button>
                    )}

                    <div className="mt-1 flex items-center justify-end space-x-1 text-[9px] text-emerald-400/80">
        ...
        >
          <input
            ...
            className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-white focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
          />
          <button
            type="submit"
            className="p-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-xs cursor-pointer"
          >
